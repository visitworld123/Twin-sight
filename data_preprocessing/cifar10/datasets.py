...
class CIFAR10_truncated(data.Dataset):
    ...
    def __init__(self, root, dataidxs=None, train=True, transform=None, target_transform=None, download=False):
        '''
        root: dataset的dir
        dataidxs: 取dataset里dataidexs的数据，即一部分
        train：True means training set, False means test set
        transform is the  data_transform
        target_transform： label的transform
        download： True means download from Internet/False means we can get it locally
        '''
        self.root = root
...
class Dataset_Personalize4BYOL(data.Dataset):
...
class Dataset_Personalize4BYOL_FMNIST(data.Dataset):
...
class Dataset_Personalize4PartialLebel(data.Dataset):

    def __init__(self, data, pseudo_targets, targets, transform=None, target_transform=None):
    ...
    def __getitem__(self, index):
        ...
        Returns:
            tuple: (image, targets) where targets is index of the targets class.
        """
        img, pseudo_targets, targets = self.data[index], self.pseudo_targets[index], self.targets[index]
        ...
        return img1, img2, pseudo_targets, targets

    def __len__(self):
        return len(self.data)

class Dataset_Personalize4PartialLebel_FMNIST(data.Dataset):
