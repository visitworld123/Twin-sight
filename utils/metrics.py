...
class Metrics(object):
    ...
    def __init__(self, topks=[1], task="classification"):  # top k means 前k个概率最大的类别中有ground truth
        self.task = task
        self.topks = topks
    ...
    def get_callbacks(self, topks, task,
        ...
        if task == "classification":
            self.metric_names = ["Acc{}".format(topk) for topk in topks] # 从top 1 到top k的Acc
...
# got it top_k准确率从0——k保存在metric_stat中
    def _classification_metric(self, loss, output, target, **kwargs):
        ...
        _, pred = output.topk(maxk, 1, True, True)    # tensor.topk(k, dim=None, largest=True, sorted=True, *, out=None) dim在指定维度排，largest=True： 是否按照从大到小的顺序排序
        # sorted：控制是否按排序顺序返回元素（如果为False，则只返回k个值，但这k个值并不会排序）
        if "pred_shift" in kwargs:
            pred_shift = kwargs["pred_shift"]
